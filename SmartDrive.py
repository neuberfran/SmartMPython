...
class SmartDrive(OpenElectrons_i2c):
    ...
    SmartDrive_PPR = 360                       # PPR - pulses per rotation
    ...
    SmartDrive_STATUS_M1   = 0x5A SmartDrive_STATUS_M1   = 0x5A
...
#            return ""
    ...
    def SmartDrive_Run_Degrees(self, motor_number, direction, speed, degrees, wait_for_completion, next_action):
        ...
        try:
            ...
            if ( direction == self.SmartDrive_Direction_Forward ):
                d = self.DegCalc(degrees)
            if ( direction != self.SmartDrive_Direction_Forward ):
                d = self.DegCalc(degrees) * -1 
    ...
    def SmartDrive_Run_Rotations(self, motor_number, direction, speed, rotations, wait_for_completion, next_action):
        ...
        try:
            ...
            if ( direction == self.SmartDrive_Direction_Forward ):
                d = self.DegCalc(rotations) * 360
            if ( direction != self.SmartDrive_Direction_Forward ):
                d = (self.DegCalc(rotations) * 360) * -1 
    ...
    ## Waits until the specified tacheometer count for the motor(s) to run is reached.
    #  @param self The object pointer.
    #  @param motor_number Number of the motor(s) to wait for.
    ...
    ## Checks to ensure the specified tacheometer count for the motor(s) to run is reached.
    #  @param self The object pointer.
    #  @param motor_number Number of the motor(s) to check. 
    ...
    ## Calculates the encoder value from user-defined degrees and encoder pulses per revolution
    #  @param self The object pointer.
    #  @param degrees The user-defined degree with respect to 360.
    def DegCalc(self, degrees):
        try:
            TPD = self.SmartDrive_PPR/360
            tickCount = degrees * TPD
            return tickCount
        except:
            pass 